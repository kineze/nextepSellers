  ...
  <section class="mx-3 mt-3 mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="space-y-3">
      <div>
        ...
        <h1 class="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Rejected Orders</h1>
      </div>

      <admin-global-filter-bar
        context-key="admin-rejected-orders"
        @filters-changed="onGlobalFiltersChanged"
      />
    </div>

    <div class="mt-4 flex flex-wrap items-center justify-between gap-2">
      <p class="text-xs text-slate-500 dark:text-slate-400">Total {{ meta.total }} rejected orders</p>
      <div class="flex items-center gap-2">
        <select
          ...
          @change="applyFilters"
        >
          <option value="rejected_date">Rejected Date</option>
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
        <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800 dark:text-slate-300">
          <tr>
            ...
            <th class="px-3 py-3 text-left">Rejected Date</th>
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
          <tr v-if="loading">
            <td colspan="9" class="px-3 py-8 text-center text-slate-500 dark:text-slate-400">Loading rejected orders...</td>
          </tr>
          <tr v-else-if="!orders.length">
            <td colspan="9" class="px-3 py-8 text-center text-slate-500 dark:text-slate-400">No rejected orders found.</td>
          </tr>
          ...
          <tr v-for="order in orders" :key="order.id" class="bg-white dark:bg-slate-900/40">
            ...
            <td class="px-3 py-3 align-top text-slate-700 dark:text-slate-200">{{ order.city?.name_en || '-' }}</td>
            <td class="px-3 py-3 align-top text-slate-600 dark:text-slate-300">{{ formatDate(order.order_datetime) }}</td>
            <td class="px-3 py-3 align-top text-slate-600 dark:text-slate-300">{{ formatDate(order.updated_at) }}</td>
...
const filters = reactive({
  ...
  date_to: '',
  seller_id: null,
  date_basis: 'rejected_date',
  page: 1,
  per_page: 20,
})

const storageKey = 'nextep-preferences-admin-rejected-orders'
...
const loadPreferences = () => {
  try {
    ...
    if (parsed?.date_basis === 'order_date' || parsed?.date_basis === 'rejected_date') {
...
const fetchOrders = async () => {
  loading.value = true
  try {
    const { data } = await axios.get('/api/admin/orders/rejected', {
      params: {
    ...
    toast.error(error?.response?.data?.message || 'Failed to load rejected orders.')
  } finally {
    loading.value = false
  }
}
