<template>
  <div class="p-3 w-full">
    <div class="mb-6">
      ...
      <h2 class="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">Delivery Fee Management</h2>
      <p class="mt-1 text-sm text-slate-500 dark:text-slate-300">Maintain delivery fees and choose the default fee.</p>
    </div>
        ...
        <table class="w-full text-left text-sm text-slate-700 dark:text-slate-200">
          ...
          <tbody class="divide-y divide-slate-200/70 dark:divide-slate-800/70">
            <tr v-if="items.length === 0">
              <td colspan="3" class="px-3 py-6 text-center text-slate-500 dark:text-slate-400">No delivery fees found</td>
            </tr>
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
              ...
              <td class="px-3 py-4">
                <span
                  ...
                  :class="item.is_default ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200' : 'bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-200'"
                >
      ...
      <div v-if="showDrawer" class="fixed inset-0 z-[1300] flex justify-end bg-slate-900/60 backdrop-blur-sm">
        <div class="flex h-screen w-full max-w-md flex-col border-l border-slate-200/70 bg-white/95 p-6 shadow-2xl dark:border-slate-800/70 dark:bg-slate-900/95">
          <div class="mb-4 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ editingId ? 'Edit Delivery Fee' : 'Create Delivery Fee' }}</h3>
          ...
          <form @submit.prevent="save" class="space-y-5">
            <div>
              <label class="mb-1 block text-sm font-semibold text-slate-700 dark:text-slate-300">Fee</label>
              <input
                v-model="form.fee"
                type="number"
                min="0"
    ...
    <transition name="fade">
      <div v-if="showDeleteModal" class="fixed inset-0 z-[1500] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
        <div class="w-full max-w-md rounded-2xl border border-slate-200/70 bg-white/95 p-6 shadow-2xl dark:border-slate-800/70 dark:bg-slate-900/95">
          <h3 class="mb-3 text-lg font-semibold text-slate-900 dark:text-white">Delete Delivery Fee</h3>
          <p class="mb-6 text-sm text-slate-600 dark:text-slate-300">Are you sure you want to delete this fee?</p>
...
const form = ref({ fee: '', is_default: false })
...
const fetchItems = async (page = 1) => {
  try {
    const { data } = await axios.get('/api/delivery-fees', {
      params: {
        page,
    ...
    toast.error(error?.response?.data?.message || 'Failed to load delivery fees.')
  }
}
...
const openDrawer = (item = null) => {
  if (item) {
    ...
    form.value = {
      fee: Number(item.fee || 0).toFixed(2),
      is_default: !!item.is_default,
    }
  } else {
    editingId.value = null
    form.value = { fee: '', is_default: false }
  }
...
const save = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await axios.put(`/api/delivery-fees/${editingId.value}`, payload)
      toast.success('Delivery fee updated.')
    } else {
      await axios.post('/api/delivery-fees', payload)
    ...
    toast.error(error?.response?.data?.message || 'Failed to save delivery fee.')
  } finally {
    saving.value = false
  }
}
...
const remove = async () => {
  ...
  try {
    await axios.delete(`/api/delivery-fees/${deletingItem.value.id}`)
