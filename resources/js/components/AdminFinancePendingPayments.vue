<template>
  <section class="mx-3 mt-3 mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="space-y-3">
      <div>
        <p class="text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-300">Finance</p>
        <h1 class="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Pending Payment Orders</h1>
      </div>

      <admin-global-filter-bar
        context-key="admin-finance-pending-payments"
    ...
    <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <article class="rounded-xl border border-amber-200 bg-amber-50/70 p-3 dark:border-amber-500/30 dark:bg-amber-500/10">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">Pending Order Value</p>
        <p class="mt-1 text-lg font-bold text-amber-800 dark:text-amber-200">LKR {{ toMoney(summary.order_value) }}</p>
      </article>
      <article class="rounded-xl border border-sky-200 bg-sky-50/70 p-3 dark:border-sky-500/30 dark:bg-sky-500/10">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-sky-700 dark:text-sky-300">Pending Delivery Charges</p>
      ...
      <article class="rounded-xl border border-cyan-200 bg-cyan-50/70 p-3 dark:border-cyan-500/30 dark:bg-cyan-500/10">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-cyan-700 dark:text-cyan-300">Pending Net Sale</p>
        <p class="mt-1 text-lg font-bold text-cyan-800 dark:text-cyan-200">LKR {{ toMoney(summary.net_sale_value) }}</p>
      </article>
      <article class="rounded-xl border border-emerald-200 bg-emerald-50/70 p-3 dark:border-emerald-500/30 dark:bg-emerald-500/10">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Pending Commission Value</p>
        <p class="mt-1 text-lg font-bold text-emerald-800 dark:text-emerald-200">LKR {{ toMoney(summary.commission_value) }}</p>
      </article>
    </div>

    <div class="mt-4 flex items-center justify-between">
      <p class="text-xs text-slate-500 dark:text-slate-400">Total {{ meta.total }} pending payment orders</p>
      <button
        type="button"
    ...
    <div class="mt-4 overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
      <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
          <tr v-if="loading">
            <td colspan="9" class="px-3 py-8 text-center text-slate-500 dark:text-slate-400">Loading pending payments...</td>
          </tr>
          <tr v-else-if="!orders.length">
            <td colspan="9" class="px-3 py-8 text-center text-slate-500 dark:text-slate-400">No pending payment orders found.</td>
          </tr>
          ...
          <tr v-for="order in orders" :key="order.id" class="bg-white dark:bg-slate-900/40">
            ...
            <td class="px-3 py-3">
              <span class="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
...
const fetchOrders = async () => {
  loading.value = true
  try {
    const { data } = await axios.get('/api/admin/finance/pending-payments', {
      params: {
    ...
    toast.error(error?.response?.data?.message || 'Failed to load pending payment orders.')
  } finally {
    loading.value = false
  }
}
