<template>
  <section class="mx-3 mt-3 mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <div class="space-y-3">
      <div>
        <p class="text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-violet-600 dark:text-violet-300">Orders</p>
        <h1 class="mt-1 text-2xl font-bold text-slate-900 dark:text-white">Completed Orders</h1>
      </div>

      <admin-global-filter-bar
        context-key="admin-completed-orders"
        @filters-changed="onGlobalFiltersChanged"
      />
    </div>

    <div class="mt-4 flex flex-wrap items-center justify-between gap-2">
      <p class="text-xs text-slate-500 dark:text-slate-400">Total {{ meta.total }} completed orders</p>
      ...
      <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
        <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800 dark:text-slate-300">
          <tr>
            ...
            <th class="px-3 py-3 text-left">Completed Date</th>
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
          <tr v-if="loading">
            <td colspan="9" class="px-3 py-8 text-center text-slate-500 dark:text-slate-400">Loading completed orders...</td>
          </tr>
          <tr v-else-if="!orders.length">
            <td colspan="9" class="px-3 py-8 text-center text-slate-500 dark:text-slate-400">No completed orders found.</td>
          </tr>
          ...
          <tr v-for="order in orders" :key="order.id" class="bg-white dark:bg-slate-900/40">
            <td class="px-3 py-3 align-top font-semibold text-slate-900 dark:text-white">
              <p>#{{ order.id }}</p>
              <span class="mt-1 inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
                {{ order.status }}
              </span>
            </td>
            ...
            <td class="px-3 py-3 align-top text-slate-600 dark:text-slate-300">{{ formatDate(order.completed_at) }}</td>
...
const storageKey = 'nextep-preferences-admin-completed-orders'
...
const loadPreferences = () => {
  try {
    ...
    if (parsed?.date_basis === 'order_date' || parsed?.date_basis === 'completed_date') {
...
const fetchOrders = async () => {
  loading.value = true
  try {
    const { data } = await axios.get('/api/admin/orders/completed', {
      params: {
    ...
    toast.error(error?.response?.data?.message || 'Failed to load completed orders.')
  } finally {
    loading.value = false
  }
}
